...
add_calc_wt <- function(x, stock, units = "t") {
  ...
  if(stock %in% c("BSSC", "WBT", "EBT", "BSTC", "HYBRIDS") & !("clutch" %in% names(x))){stop("Cannot find size !!")}
  if(stock %in% c("BSSC", "WBT", "EBT", "BSTC", "HYBRIDS") & !("maturity" %in% names(x))){stop("Cannot find size !!")}
  ...
  if(stock %in% c("BSSC", "BSTC", "WBT", "EBT", "HYBRIDS")) {
    x %>%
      mutate(a = case_when(stock == "BSSC" & sex == 1 ~ 0.000267,
                           ...
                           stock == "HYBRIDS" & sex == 1 ~ 0.000267,
                           stock == "HYBRIDS" & sex == 2 & (maturity == 1 | clutch > 0) ~ 0.001158,
                           stock == "HYBRIDS" & sex == 2 & (maturity == 0 | clutch <= 0 | is.na(clutch)) ~ 0.001047,
      ...
      mutate(b = case_when(stock == "BSSC" & sex == 1 ~ 3.097253,
                           ...
                           stock == "HYBRIDS" & sex == 1 ~ 3.097253,
                           stock == "HYBRIDS" & sex == 2 & (maturity == 1 | clutch > 0) ~ 2.708793,
                           stock == "HYBRIDS" & sex == 2 & (maturity == 0 | clutch <= 0 | is.na(clutch)) ~ 2.708367,
