...
add_legal <- function(x, stock = NULL, use_observer = T, use_current = T) {
  ...
  if(use_current == T) {
    legal_sizes <- tibble(BSAIstock = c("BSSC", "HYBRIDS", "WBT", "EBT", "BBRKC", "AIGKC", "EAG", "WAG", "PIGKC",
                                        "SMBKC", "PIBKC", "PIRKC", "WAIRKC"),
                          legal_size = c(78, 78, 111, 121, 124, 135, 135, 135, 135, 124, 124, 124, 124))
    x %>% mutate(legal_size = filter(legal_sizes, BSAIstock == stock) %>% pull(legal_size)) -> x
  }
  if(use_current == F) {

    tibble(BSAIstock = c("BSSC", "HYBRID", "WBT", "EBT", "BBRKC", "AIGKC", "EAG", "WAG", "PIGKC",
                         "SMBKC", "PIBKC", "PIRKC", "WAIRKC"),
           legal_size = c(78, 78, 111, 121, 124, 135, 135, 135, 135, 124, 124, 124, 124)) %>%
